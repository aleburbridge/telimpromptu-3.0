...
export default function WaitingRoomPage() {
    ...
    return (
        <div className='container container-md'>
            <p className='text-2xl mt-2 text-white'>Waiting Room for <i>{roomName}</i></p>
            <p className='mt-4 text-white'>{readyToStart ? '💡 Once you start the game, you won\'t be able to add more players' : ''}</p>
            <button
                className='btn btn-primary mt-3 h-12'
                disabled={playerClickedStart || !readyToStart}
                onClick={handleClick}>
                {readyToStart ? '☑︎ Ready to Start' : `Need ${3 - playersList.length} more players to start`}
            </button>
            <div className='container-inner mt-2'>
                <p className='text-xl'><u>Players</u></p>
                {playersList.map(player => (
                    <div key={player.id} className='mt-1 text-white'>
